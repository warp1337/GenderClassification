// Revision history:
// [20151119_Curtis] 1. normalize and crop face images for gender classification
//                   2. use uniform LBP and SVM to train gender classifier
   ...
   {  
         ...
         if (faceNum > 0)
         {
	       ...
	       for (size_t j = 0; j < fLandmarks[i].size(); j++)
	           cv::circle(img, fLandmarks[i][j], 1, cv::Scalar(255,255,255));
            }
         }
         // (debug) show no face 
         //else
