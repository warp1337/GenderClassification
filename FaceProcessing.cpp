...
#ifdef GPU
   if (!m_faceCascade_gpu.load(faceXml))
   {
      printf("Error: cannot load xml file for face detection in function CFaceProcessing::CFaceProcessing()\n");
   }
#endif
...
int CFaceProcessing::FaceDetection_GPU(const cv::Mat colorImg)
{
   ...
   // convert to GpuMat
   cv::gpu::GpuMat skinBinImg_gpu, skinBinImg_gpu_t, m_grayImg_gpu, skinSegGrayImg_gpu, m_faces_gpu;
   skinBinImg_gpu.upload(skinBinImg);
   // erode and dilate to remove small segmentation
   cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
   cv::gpu::erode(skinBinImg_gpu, skinBinImg_gpu_t, kernel, cv::Point(-1, -1), 2);
   cv::gpu::dilate(skinBinImg_gpu_t, skinBinImg_gpu, kernel, cv::Point(-1, -1), 2);
   // apply GaussianBlur to have a complete segmentation
   ...
   m_faceCascade_gpu.findLargestObject = false;
   int detections_num = m_faceCascade_gpu.detectMultiScale(skinSegGrayImg_gpu, m_faces_gpu, 1.2, 4 , 
                                                         cv::Size(20,20));
